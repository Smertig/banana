...
static_assert(std::is_same_v<banana::api_result<banana::api::message_t, banana::agent::httplib_blocking_monadic>, banana::expected<banana::api::message_t>>);
static_assert(std::is_same_v<banana::api_result<banana::api::message_t, banana::agent::httplib_blocking>, banana::api::message_t>);

int main(int argc, const char** argv) {
    const auto [token, target, name, message_text] = parse_input_or_fail(argc, argv);

    try {
        banana::agent::httplib_blocking agent(token);
    ...
    try {
        banana::agent::httplib_blocking_monadic agent(token);
